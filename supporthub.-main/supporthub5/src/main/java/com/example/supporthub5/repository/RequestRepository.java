...
public interface RequestRepository  extends JpaRepository<Request,Long> {
    ...
    @Query(nativeQuery=true,value="SELECT  * from requests  WHERE (request_id=?1 or ?1 is null) AND (location_id=?2 or ?2 is null ) AND  (service_id=?3 or ?3 is null)  AND (description=?4 or ?4 is null )  AND  (state=?5 or ?5 is null) AND(created_on=?6 or ?6 is null)")
